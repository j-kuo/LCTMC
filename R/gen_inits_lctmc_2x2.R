...
#'
#' @description This function generates initial value for the Latent Class CTMC model (general 2x2 case). This is a two-step process, where we first use the K-means algorithm on the individuals' effect to create cluster level estimates.
...
#' @export
#'
#' @example inst/examples/ex_gen_inits_lctmc_2x2.R

gen_inits_lctmc_2x2 = function(df,
                               Xmat,
                               Wmat,
                               dt,
                               K,
  ...
  if (returns == 1L) {
    ...
    class(out) = c("lctmc_2x2.inits", "list")
    return(out)
  }
