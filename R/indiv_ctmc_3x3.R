#' @title Estimate the CTMC (3x3) parameters for each individual
#'
...
#' @param df a data frame object transformed via `fmt_rowwise_3x3trans()`
...
#' The number of columns is equal to 10, where the first nine columns are the estimated coefficients and the last columns indicate whether convergence criteria is met for each person.
  ...
  for (i in seq_along(subject_list)) {
    ...
    opt = stats::optim(
      ...
      fn = function(par) {
        ...
        P = get_P_3x3(q12 = q12, q21 = q21, q23 = q23, dt = dt.i)
