#' @title Fit a Latent Class CTMC model (3x3)
#'
#' @description A latent class CTMC (special 3x3 case) where we assume the data generating process is a CTMC when conditioned on a un-observed latent variable.
#'
...
#' \itemize{
#'   \item **inits** a list object obtained from the `gen_inits_lctmc_3x3()` function
#'   \item **EM** a list object obtained from the `EM_lctmc_3x3()` function
#'   \item **SE** a list object obtained from the `get_SE_lctmc_3x3()` function
...
#' @export
#'
#' @example inst/examples/ex_lctmc_3x3.R
...
lctmc_3x3 = function(data = data.frame(),
  ...
  if (test_if_global_optim$test && K == 3) {
    ...
    my_df.test = fmt_rowwise_3x3trans(data = data)
