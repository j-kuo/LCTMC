...
#' Note that step 2 of `gen_inits_lctmc_3x3()` will likely produce a good approximation of the MLE. so when the EM algorithm requires large number of iterations
...
#' @export
#'
#' @example inst/examples/ex_EM_lctmc_3x3.R

EM_lctmc_3x3 = function(theta.init,
                        theta.names,
                        par_constraint,
                        K,
                        df,
  ...
  bik_all.old = bik_all_3x3(theta = theta.init, data = df, Xmat = df.Xmat, Wmat = df.Wmat, dt = df.dt, K = K, P.rs = FALSE, theta.names = theta.names.bik)
